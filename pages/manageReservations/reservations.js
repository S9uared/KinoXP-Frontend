...
export async function initReservations() {
  fetchReservations();
  document
    .getElementById("phone-search-btn")
    .addEventListener("click", createReservationTable);
  document
  ...
  try {
    const tableData = filteredList
      .map((reservation) => {
        ...
        let showing = fetch(showingUrl)
          .then(handleHttpErrors)
          .then((data) => {
            return data;
          });
        const movieUrl = API_URL + "/movies/" + showing.movieId;
        let movie = fetch(movieUrl)
          .then(handleHttpErrors)
          .then((data) => {
            return data;
          });
        ...
        const seatPositions = reservation.seats
...
function filterByPhone() {
  ...
  const filteredReservations = reservationList.filter(
    (reservation) => phoneNumber === reservation.customerInfo.phoneNumber
  );
...
function resetSearch() {
  document.getElementById("search-prompt").innerText =
    "Search for a phone number to find reservations";
